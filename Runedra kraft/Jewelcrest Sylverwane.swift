import SwiftUI
@available(iOS 15.0,*)
struct JewelcrestSylverwaneView: View {
    // MARK: - Updated Enums
    enum InfusionType: String, CaseIterable {
        case basic = "Opal Protocol"
        case enhanced = "Sapphire Protocol"
        case quantum = "Diamond Protocol"
        case singularity = "Onyx Protocol"
    }
    
    enum MaterialType: String, CaseIterable {
        case alpha = "Ruby Matrix"
        case beta = "Emerald Composite"
        case gamma = "Topaz Alloy"
    }
    
    enum IntensityLevel: String, CaseIterable {
        case minimal = "Soft Glow"
        case moderate = "Brilliant"
        case extreme = "Radiant"
    }
    
    // MARK: - State Variables
    @State private var coreValue: Double = 50
    @State private var infusionRate: Double = 2.5
    @State private var gemstoneFactor: String = ""
    @State private var prismConstant: String = ""
    ...
    private var backgroundGradient: some View {
        LinearGradient(
            gradient: Gradient(colors: [Color(.systemTeal).opacity(0.2), Color(.systemPurple).opacity(0.2)]),
    ...
    private var headerView: some View {
        HStack {
            VStack(alignment: .leading) {
                Text("Jewelcrest Sylverwane")
                    .font(.largeTitle)
                    .fontWeight(.bold)
                    .foregroundColor(.purple)
                
                Text("Gemstone resonance infusion system")
                    .font(.subheadline)
            ...
            Menu {
                Button(action: captureScreenshot) {
                    Label("Capture Facet", systemImage: "camera.fill")
                }
                ...
                if infusionResult != nil {
                    ...
                    Button(action: prepareShare) {
                        Label("Share Radiance", systemImage: "square.and.arrow.up")
                    }
                }
                
                Button(action: showHelp) {
                    Label("Gem Guide", systemImage: "questionmark.circle")
                }
            } label: {
    ...
    private var parametersSection: some View {
        CardView(title: "Gemstone Parameters", tag: 1) {
            // Core Value as Stars
            VStack(alignment: .leading) {
                HStack {
            ...
            ParameterTextField(value: $gemstoneFactor, label: "Gemstone Factor", symbol: "function")
            
            ParameterTextField(value: $prismConstant, label: "Prism Constant", symbol: "ruler")
            
            HStack {
                Text("Facets: \(iterations)")
                    .font(.headline)
    ...
    private var configurationSection: some View {
        CardView(title: "Gem Configuration", tag: 2) {
            MenuPickerView(selection: $infusionType, title: "Infusion Protocol", options: InfusionType.allCases)
            
            MenuPickerView(selection: $materialType, title: "Gem Matrix", options: MaterialType.allCases)
    ...
    private func resultsSection(result: InfusionResult) -> some View {
        CardView(title: "Gemstone Results", tag: 2) {
            VStack(alignment: .leading, spacing: 15) {
                HStack {
                    Text("Radiance Report")
                        .font(.title2)
                        .fontWeight(.semibold)
                ...
                ResultRow(icon: "waveform.path.ecg", label: "Clarity Index", value: "\(String(format: "%.1f", result.stabilityIndex))/10", color: result.stabilityIndex > 7 ? .green : result.stabilityIndex > 4 ? .orange : .red)
                ...
                VStack(alignment: .leading, spacing: 8) {
                    Text("Gemologist's Notes")
                        .font(.headline)
                    
                    if result.stabilityIndex < 5 {
                        Text("• Increase prism constant")
                        Text("• Reduce luminance rate by 10-15%")
                        Text("• Consider using Emerald Composite")
                    } else if result.energyOutput < 50 {
                        Text("• Increase core brilliance")
                        Text("• Try Diamond Protocol")
                        Text("• Consider Radiant intensity")
                    } else {
                        Text("• Parameters are optimal")
                        Text("• Consider adding more facets")
                    }
                }
                .padding()
    ...
    struct SparkleFrequencyControl: View {
        ...
        var label: String = ""
        var symbol: String = "diamond.fill"
        ...
        private var unit: String {
            if label == "Core Brilliance" {
                return " carats"
            } else if label == "Luminance Rate" {
                return " lux"
            } else {
                return ""
            }
    ...
    struct InfusionResult {
        ...
        var shareableText: String {
            """
            Jewelcrest Sylverwane Results
            ----------------------------
            Luminous Output: \(String(format: "%.2f", energyOutput)) MW
            Clarity Index: \(String(format: "%.1f", stabilityIndex))/10
    ...
    private func validateAndStartInfusion() {
        guard !gemstoneFactor.isEmpty else {
            showAlert(message: "Please enter Gemstone Factor", type: .warning)
            return
        }
        
        guard !prismConstant.isEmpty else {
            showAlert(message: "Please enter Prism Constant", type: .warning)
            return
        }
    ...
    private func startInfusion() {
        ...
        let stabilityBase = (Double(gemstoneFactor) ?? 5.0) * (Double(prismConstant) ?? 1.0)
        ...
        Core Brilliance: \(Int(coreValue)) carats
        Luminance Rate: \(String(format: "%.1f", infusionRate)) lux
        Gemstone Factor: \(gemstoneFactor)
        Prism Constant: \(prismConstant)
        Facets: \(iterations)
        Infusion Protocol: \(infusionType.rawValue)
        Gem Matrix: \(materialType.rawValue)
        Luminance: \(intensityLevel.rawValue)
        """
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.5) {
            ...
            showAlert(message: "Gemstone infusion completed successfully!", type: .success)
        }
    }
    ...
    private func captureScreenshot() {
        ...
        capturedImage = image
        showShareSheet = true
        showAlert(message: "Facet captured!", type: .success)
    }
    ...
    private func showHelp() {
        showAlert(message: """
        Jewelcrest Sylverwane Guide:
        1. Set all gemstone parameters
        2. Configure infusion settings
        3. Begin the infusion process
        4. View radiance report & notes
        """, type: .info)
    }
}

@available(iOS 15.0,*)
struct JewelcrestSylverwaneView_Previews: PreviewProvider {
